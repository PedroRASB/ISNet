...
class ISNetGradInputLgt(pl.LightningModule):
    def __init__(self,multiLabel=False,multiMask=False,
                 ...
                 cut=1,cut2=25,A=1,B=3,d=0.996,E=1):
        
        super (ISNetGradInputLgt,self).__init__()
    ...
    def compound_loss(self,outputs,labels,inputs=None,masks=None,val=False):
        ...
        for i in list(range(outputs.shape[-1])):
            ...
            heatmaps=ag.grad(logits, inputs, retain_graph=True, create_graph=True)
            for i,H in enumerate(heatmaps,0):
                grads.append(H)
        grads=torch.stack(grads,1)

        tmp=inputs.unsqueeze(1).repeat(1,grads.shape[1],1,1,1)
        grad=grads*tmp
