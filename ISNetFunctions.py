...
def LRPLossActivated (heatmap, mask,device,eps=1e-4,E=1):
    ...
    with torch.cuda.amp.autocast(enabled=False):
        ...
        ones=torch.ones(mask.shape).to(device)
        Imask=ones-mask
        ...
        #target: zeros
        one=torch.ones(l.shape).to(device)
...
def LRPDenseReLU(layer,rK,aJ,aK,e,device):
    ...
    c=nn.functional.linear(s,weight=W)
    #unmix:
...
def LRPConvReLU(layer,rK,aJ,aK,e,device):
...
def AvgPoolWeights(kernel_size,stride,device,channels):
    ...
    biases=torch.zeros((channels)).to(device)  
...
def SumPoolWeights(kernel_size,stride,device,channels):
...
def LRPPool2d(layer,rK,aJ,aK,e,device,adaptative=False):
    ...
    #avgpooling weights
    weights,stride,_=AvgPoolWeights(kernel_size,stride,device=device,channels=channels)
...
def FuseBN(layerWeights, BN, aKConv, device,training,Ch0=0,Ch1=None,layerBias=None,bias=True):
    ...
    #device='cpu' or 'cuda:0', 'cuda:1',..
    #training: True during traning, False during evaluation
    ...
    if(bias):
        if(layerBias is None):
            layerBias=torch.zeros((weights.shape[0])).to(device)
        if(BN.weight is None):
            biases=layerBias*0
        else:
...
def LRPPool2dBNReLU(layer,BN,rK,aJ,aK,e,device,aKPool,training,Ch0=0,Ch1=None):
    ...
    weights,stride,biases=AvgPoolWeights(layer.kernel_size,layer.stride,
                                         device=device,channels=channels)
    
    #consider BN:
    weights,biases=FuseBN(layerWeights=weights, BN=BN, aKConv=aKPool,bias=True,
                          device=device,training=training,Ch0=Ch0,Ch1=Ch1,layerBias=biases)
...
def MultiBlockPoolBNReLU(layer,BN,rK,aJ,e,device,aKPool,aK,training,Ch0=0,Ch1=None):
    #propagates relevance through average pooling followed by batch normalization and ReLU 
    ...
    weights,stride,biases=AvgPoolWeights(layer.kernel_size,
                                         layer.stride,device=device,channels=channels)
    ...
    for i,norm in enumerate(BN,0):
        w,b=FuseBN(layerWeights=weights,layerBias=biases, BN=BN[i], aKConv=aKPool,
                   device=device,training=training,Ch0=Ch0,Ch1=Ch1,bias=True)
        W.append(w)
        B.append(b)
...
def MultiLayerConvBNReLU(layer,BN,rK,aK,aJ,e,device,aKConv,training,Ch0=0,Ch1=None):
...
def LRPConvBNReLU(layer,BN,rK,aJ,aK,e,device,aKConv,training,Ch0=0,Ch1=None):
    ...
    weights,biases=FuseBN(layerWeights=layer.weight, BN=BN, aKConv=aKConv,
                   device=device,training=training,Ch0=Ch0,Ch1=Ch1,layerBias=layer.bias,
                    bias=True)
...
def w2RuleInput(layer,rK,aJ,aK,device,e):
    ...
    B2=torch.pow(bias,2)
    AK=nn.functional.conv2d(torch.ones(aJ.shape).to(device),weight=W2,
...
def w2BNRuleInput(layer,BN,rK,aJ,aK,e,device,aKConv,training):
...
def ZbRuleConvBNInput(layer,BN,rK,aJ,aK,aKConv,e,device,l=0,h=1,training=True,op=1,Zb0=True):
    ...
    #training: True during traning, False during evaluation
    #BN: batch normalization layer
    #op: addiotional parameter to match input shape of convolution with the equivalent transpose convolution
    ...
    weights,biases=FuseBN(layerWeights=layer.weight, BN=BN, aKConv=aKConv,
                          device=device,training=training,layerBias=layer.bias,
                          bias=True)
        

    #positive and negative weights:
    WPos=torch.max(weights,torch.zeros(weights.shape).to(device))
    WNeg=torch.min(weights,torch.zeros(weights.shape).to(device))
    #positive and negative bias:
    BPos=torch.max(biases,torch.zeros(biases.shape).to(device))
    BNeg=torch.min(biases,torch.zeros(biases.shape).to(device))
        
    #propagation:
    aKPos=nn.functional.conv2d(torch.ones(aJ.shape).to(device)*l,weight=WPos,
                                bias=BPos*l,stride=layer.stride,padding=layer.padding)
    aKNeg=nn.functional.conv2d(torch.ones(aJ.shape).to(device)*h,weight=WNeg,
...
def ZbRuleConvInput(layer,rK,aJ,aK,e,device,l=0,h=1):
    ...
    if(layer.bias is not None):
        ...
        biases=torch.zeros(layer.out_channels).to(device)
    ...
    c=nn.functional.conv_transpose2d(s,weight=weights,bias=None,
                                        stride=layer.stride,padding=layer.padding,output_padding=0)
    cPos=l*nn.functional.conv_transpose2d(s,weight=WPos,bias=None,stride=layer.stride,
...
def LRPMaxPool2d(layer,rK,aJ,aK,e,device):
    #propagates relevance through max pooling followed by ReLU 
    #layer: pooling layer throgh which we propagate relevance
    #e: LRP-e term. Use e=0 for LRP0
    #rK: relevance at layer L ReLU output
    #aJ: values at layer L pooling input
...
def MultiBlockMaxPoolBNReLU(layer,BN,rK,aJ,aK,e,device,aKPool,training,Ch0=0,Ch1=None):
    ...
    stride=(1,1)
    bias=torch.zeros(channels).to(device)
    weights=torch.zeros(channels,channels,1,1).to(device)
    for i in range(channels):
        weights[i,i,:,:]=torch.ones(1,1).to(device)
